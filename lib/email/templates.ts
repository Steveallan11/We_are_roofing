...
export function variationSentEmail(props: {
  ...
  const messageHtml = props.messageBody?.trim()
    ...
    : paragraphsToHtml("We have prepared a quotation for the additional work discussed. Please use the button below to review the scope and confirm whether you would like us to proceed.");
  return shell(
...
export function variationQuoteSentEmail(props: {
  ...
  const messageHtml = props.messageBody?.trim()
    ? paragraphsToHtml(props.messageBody)
    : paragraphsToHtml("We have prepared one combined quotation covering the additional work discussed. Please use the button below to review each item and confirm whether you would like us to proceed.");
...
export function invoiceSentEmail(props: {
  ...
  return shell(
    ...
    `
      ...
      <div style="background:#0a0a0a;border-radius:8px;padding:18px 20px;margin:22px 0 0;text-align:center">
        <div style="font-size:13px;color:#f4efe2;line-height:1.6;margin-bottom:14px">
          Open the invoice to view the full document and payment details.
        </div>

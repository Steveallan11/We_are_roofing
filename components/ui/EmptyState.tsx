...
export function EmptyState({ title, message, actionLabel, actionHref }: Props) {
  return (
    <div className="card flex flex-col items-center justify-center p-8 text-center">
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-[var(--border)] bg-black/20 text-2xl text-[var(--text-faint)]">+</div>
      ...
      {actionLabel && actionHref ? (
        <Link className="button-primary mt-5 w-full sm:w-auto" href={actionHref as Route}>
          {actionLabel}
        </Link>
      ) : null}
    </div>
